...
# ensure repo root
if [ ! -d ".git" ]; then
  echo "❌ Please run from your repo root (where .git exists)."; exit 1
fi
...
# 1) Archive any old Cost Planner files (skip v2)
ARCHIVE_DIR="archive/cost_planner_old_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$ARCHIVE_DIR"
echo "🔎 Archiving old Cost Planner pages to $ARCHIVE_DIR …"
moved=0
# capture candidates (avoid glob errors in zsh)
while IFS= read -r -d '' f; do
  case "$f" in *"/cost_planner_v2/"*) continue ;; esac
  mkdir -p "$ARCHIVE_DIR/$(dirname "$f")"
  if git ls-files --error-unmatch "$f" >/dev/null 2>&1; then
    git mv "$f" "$ARCHIVE_DIR/$(dirname "$f")/"
  else
    mv "$f" "$ARCHIVE_DIR/$(dirname "$f")/"
  fi
  echo "  • archived: $f"; moved=1
done < <(find pages -type f -name '*cost_planner*.py' -print0 2>/dev/null || true)
if [ $moved -eq 0 ]; then echo "ℹ️  No old Cost Planner files found to archive."; fi

# 2) Create v2 package dirs
...
# 9) Byte-compile to catch syntax issues early
...
# 10) Commit
git add -A
git commit -m "feat(cp-v2): archive old cost planner pages; scaffold PFMA-styled v2 stubs + helpers" || true
